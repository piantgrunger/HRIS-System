...
/**
 * Class m190218_021754_alterhitung
 */
class m190218_021754_alterhitung extends Migration
{
    /**
     * {@inheritdoc}
     */
...
BEGIN
         ...
         create temporary table tp_banding (
          ...
          and p.id_satuan_kerja=id_satuan_kerja;
           ...
           insert into tb_dt_hitung_tunjangan(id_hitung_tunjangan,id_pegawai,jumlah_absen,total_jam_potong,tunjangan_tpp,capaian_kinerja,tambahan_tpp,total_tunjangan,total_banding)
            select id, p.id_pegawai,sum(case when ja.nama_jenis_absen<>'Libur' then 1 else 0 end),sum(coalesce(a.total_jam_potong,0)),coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan),100,j1.tambahan_tpp,
        case when  ((coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan)) +( coalesce(j1.tunjangan_tpp/2, j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan) *100/100 ))
          -(sum(coalesce(a.total_jam_potong,0))*0.635/100*(coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan))+coalesce(j1.tunjangan_tpp/2,j.tambahan_tunjangan_kinerja,0))
          < 0 then 0 else
          ((coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan)) +(coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan) *100/100 ))
          -(sum(coalesce(a.total_jam_potong,0))*0.635/100*(coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan)))
           end+coalesce(j1.tambahan_tpp,0)
           +((coalesce(a1.total_jam_potong,0)) *0.635/100*(coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan)))
			  ,((coalesce(a1.total_jam_potong,0)) *0.635/100*(coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan)))
          ...
          group by p.id_pegawai,coalesce(j1.tunjangan_tpp/2,j.ikkd*j.nilai_jabatan,j.tpp_statis,g.ikkd*g.nilai_jabatan);
    ...
    {
        echo "m190218_021754_alterhitung cannot be reverted.\n";
